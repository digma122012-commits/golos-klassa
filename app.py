...
def index():
    ...
    if query:
        sql = """
            SELECT id, text, theme, custom_theme, votes, image_data, poll_options 
            FROM ideas 
            WHERE 
                text LIKE ? 
                OR theme LIKE ?
    ...
    <head>
        ...
        <style>
            * {{ box-sizing: border-box; }}
            ...
            form {{ margin: 20px 0; }}
            select, input[type="text"], textarea, input[type="file"], button {{ width: 100%; padding: 12px; margin: 8px 0; border: 1px solid #ddd; border-radius: 8px; font-size: 16px; }}
...
def add_idea():
    ...
    conn = sqlite3.connect(DB_PATH)
    conn.execute("""
        INSERT INTO ideas (text, theme, custom_theme, poll_options, ip, image_data) 
        VALUES (?, ?, ?, ?, ?, ?)
    """, (text, theme, custom_theme, json.dumps(poll_options) if poll_options else None, user_ip, image_data))
    conn.commit()
    conn.close()
    return redirect('/')
...
def vote(idea_id):
    ...
    if not exists:
        ...
        conn.commit()
    conn.close()
    return redirect('/')  # остаёмся на главной
